...
const sumPolls = (
  pollA: { [key: string]: Number },
  pollB: { [key: string]: Number }
): { [key: string]: Number } => {
...
const getMinPlayers = (element: Element): Number =>
  Number(element.getElementsByTagName("stats")[0].getAttribute("minplayers"));

const getMaxPlayers = (element: Element): Number =>
